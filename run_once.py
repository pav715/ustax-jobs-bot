...
US_TAX_TERMS = re.compile(
    ...
    r"tax\s*reviewer|tax\s*associate|tax\s*advisor|tax\s*manager|"
    r"tax\s*software|tax\s*sme|tax\s*subject|"
    r"senior\s*tax|tax\s*accountant|"
    ...
    r"senior\s*associate|senior\s*specialist|"
    r"assistant\s*manager|tax\s*operations|"
    r"us\s*taxation|us\s*entity\s*tax|"
    r"cross.?border\s*tax|"
    r"tax\s*intern|tax\s*assistant|junior\s*tax|"
    ...
    r"transfer\s*pricing|tax\s*risk\s*assessment|"
    ...
    r"strategic\s*advisory|tax\s*advisory"
    r")",
    re.IGNORECASE,
)
