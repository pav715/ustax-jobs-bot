...
US_TAX_TERMS = re.compile(
    ...
    r"tax\s*senior|senior\s*tax|tax\s*manager|tax\s*accountant|"
    r"international\s*tax(ation)?|expat(riate)?\s*tax(ation)?|"
    r"global\s*mobility\s*tax|"
    r"enrolled\s*agent|\bEA\b\s*\(?tax|cpa\s*(us\s*)?tax|"
    r"turbo\s*tax|turbotax|ultratax|ultra\s*tax|drake\s*(tax|software)|"
    r"proseries|pro\s*series|proconnect|lacerte|gosystem|go\s*system|"
    r"cch\s*axcess|cch\s*prosystem|"
    r"tax\s*returns?"
    r")\b",
    re.IGNORECASE,
)
